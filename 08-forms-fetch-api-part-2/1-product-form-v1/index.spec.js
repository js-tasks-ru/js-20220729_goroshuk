...
describe("forms-fetch-api-part-2/product-form-v1", () => {
  ...
  it("should render categories data correctly", () => {
    const subcategory =
      productFormComponent.element.querySelector("#subcategory");
    ...
    expect(
      subcategory.children[subcategory.children.length - 1]
    ).toHaveTextContent(categoriesNames[categoriesNames.length - 1]);
  });

  it("should render product data correctly", () => {
    const { productForm, imageListContainer } =
      productFormComponent.subElements;
    ...
    expect(values.title).toBe(
      '10.1" Планшет Lenovo Tab E10 TB-X104L 32 ГБ 3G, LTE черный'
    );
    expect(values.quantity).toBe("73");
    expect(values.price).toBe("10");
    expect(values.discount).toBe("21");
    expect(values.images[0].url).toBe(
      "https://shop-image.js.cx/101-planset-lenovo-tab-e10-tb-x104l-32-gb-3g-lte-cernyj-8.jpg"
    );
  });

  it('should dispatch "product-updated" event after product creating', async () => {
    const spyDispatchEvent = jest.spyOn(
      productFormComponent.element,
      "dispatchEvent"
    );
